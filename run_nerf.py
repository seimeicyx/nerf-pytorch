...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    if use_viewdirs:
        ...
        if c2w_staticcam is not None:
            ...
            from utils.camera import SceneMeta,PinholeCamera
            cam=PinholeCamera(scene_meta=SceneMeta(H,W,K),camera_pose=c2w_staticcam)
            rays_o, rays_d = cam.cast_ray()
...
def create_nerf(args):
    ...
    embed_fn=FreEncoder(args.multires)
    ...
    if args.use_viewdirs:
        embeddirs_fn=FreEncoder(args.multires_views)
        input_ch_views=embeddirs_fn.out_dim
        # embeddirs_fn, input_ch_views = get_embedder(args.multires_views, args.i_embed)
    output_ch = 5 if args.N_importance > 0 else 4
    skips = [4]
    from models.nerf import NeRF as _NeRF
    # model = NeRF(D=args.netdepth, W=args.netwidth,
    #              input_ch=input_ch, output_ch=output_ch, skips=skips,
    #              input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
    model = _NeRF(D=args.netdepth, W=args.netwidth,
                 pts_ch=input_ch, skips=skips,
                 view_ch=input_ch_views).to(device)
    ...
    if args.N_importance > 0:
        ...
        #                   input_ch=input_ch, output_ch=output_ch, skips=skips,
        #                   input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
        model_fine = _NeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                          pts_ch=input_ch, skips=skips,
                          view_ch=input_ch_views).to(device)
    ...
    from models.nerf import apply_model
    network_query_fn = lambda inputs, viewdirs, network_fn :apply_model(inputs,viewdirs,embed_fn,embeddirs_fn,network_fn,args.netchunk)
    # Create optimizer
...
#     raw = run_network(pts)
    ...
    if N_importance > 0:
        ...
        from utils.render import sample_pdf as _sample_pdf
        z_samples = _sample_pdf(z_vals_mid, weights[...,1:-1], N_importance,pytest)
...
def train():
    ...
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = nerf.create_nerf(args)
    ...
    if use_batching:
        ...
        print('get rays')
        from utils.camera import SceneMeta,PinholeCamera
        rays = np.stack([PinholeCamera(scene_meta=SceneMeta(H,W,K),camera_pose=p).cast_ray() for p in poses[:,:3,:4]], 0) # [N, ro+rd, H, W, 3]
